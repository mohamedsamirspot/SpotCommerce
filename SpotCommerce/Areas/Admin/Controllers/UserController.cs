...
namespace SpotCommerce.Areas.Admin.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> EditUserRolePost(string id, string rdUserRole)
        {
            var user = await _db.ApplicationUser.FirstOrDefaultAsync(u => u.Id == id);
            var role = await _db.UserRoles.FirstOrDefaultAsync(u => u.UserId == id);
            var r = await _db.Roles.FirstOrDefaultAsync(u => u.Id == role.RoleId);

            
            await _userManager.RemoveFromRoleAsync(user, r.Name);


            if (rdUserRole == SD.OperatorUser)
            {
                await _userManager.AddToRoleAsync(user, SD.OperatorUser);
            }
            else
            {
                if (rdUserRole == SD.FrontDeskUser)
                {
                    await _userManager.AddToRoleAsync(user, SD.FrontDeskUser);
                }
                else
                {
                    if (rdUserRole == SD.ManagerUser)
                    {
